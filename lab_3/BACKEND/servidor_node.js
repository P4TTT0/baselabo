...
//AGREGAR
app.post("/agregarArticuloBD", upload.single("foto"), verificar_jwt, (request, response) => {
    let obj_respuesta = {
        exito: false,
    ...
    let path = file.destination + Articulo_json.precio + "." + extension;
    fs.renameSync(file.path, path);
    Articulo_json.path_foto = path.split("public/")[1];
    request.getConnection((err, conn) => {
        if (err)
            throw "Error al conectarse a la base de datos.";
        conn.query("INSERT INTO articulos set ?", [Articulo_json], (err, rows) => {
            if (err) {
                console.log(err);
                throw "Error en consulta de base de datos.";
            }
...
app.delete("/artis", verificar_jwt, (request, response) => {
    ...
    request.getConnection((err, conn) => {
        ...
        conn.query("SELECT path_foto FROM articulos WHERE id = ?", [obj.id], (err, result) => {
            if (err)
                throw "Error en consulta de base de datos.";
            if (result.length != 0) {
    ...
    request.getConnection((err, conn) => {
        if (err)
            throw "Error al conectarse a la base de datos.";
        conn.query("DELETE FROM articulos WHERE id = ?", [obj.id], (err, rows) => {
            if (err) {
                console.log(err);
                throw "Error en consulta de base de datos.";
            }
...
//MODIFICAR
app.post("/artis", upload.single("foto"), verificar_jwt, (request, response) => {
    let obj_respuesta = {
        exito: false,
    ...
    let path = file.destination + articulo.tipo + "_modificacion" + "." + extension;
    fs.renameSync(file.path, path);
    ...
    request.getConnection((err, conn) => {
        if (err)
            throw "Error al conectarse a la base de datos.";
        conn.query("UPDATE articulos set ?  WHERE id = ?", [articulo_modif, articulo.id_articulo], (err, rows) => {
            if (err) {
                console.log(err);
                throw "Error en consulta de base de datos.";
            }
